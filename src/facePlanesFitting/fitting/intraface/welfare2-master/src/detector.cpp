...
Mat Detector::detect(const string imgname, int numLandmarks){
	...
	if (dtype == DOPENCV){
		detected_faces  = cvHaarDetectObjects(frame_bw, HaarCascade, storage, search_scale_factor, 2, flags, minFeatureSize);
	}
	else{
	...
	int iface = 0;
	// r now stores a pointer to a list of sequential elements, each of which is a face.
...
/* Modified by Visesh Chari 6th Feb 2015. I write comments to understand this code and modify the
 * program to detect / select landmarks that correspond to Zhu Ramanan and then output them.
 */
...
Mat Detector::detectNorm(const string filename, const float faceWidth, const float faceHeight, const float patchSize, Mat& landmarks, int numLandmarks, bool mirror, bool showLandmark){
    ...
    // detection done using Haar detectors.

	gettimeofday(&end, NULL);	// timing function
    double elapsed = (end.tv_sec - begin.tv_sec) + 
              ((end.tv_usec - begin.tv_usec)/1000000.0);
	...
	{ // for each face, do
		...
		// detect landmarks (function inside if condition does the detection)
		if ( faceLandmark->Detect( frame_mat, rect, landmarks, score ) == INTRAFACE::IF_OK )
		{
			...
			if (score >= notFace) {
				...
				// if there is a requirement to draw the landmarks on the face.
				if (showLandmark){
					...
					for (int i = 0 ; i < landmarks.cols ; i++)
					{
						...
						// output point coordinates for separate verification.
						cout << landmarks.at<float>(0, i) << " " << landmarks.at<float>(1, i) << endl ;
					}
				}
				isdetect = true;
				break;
    ...
    /************* Code for detecting landmarks on face is done ************/
	
	// if no faces have been detected, just exit.
	if ( !isdetect ){
		cvReleaseMemStorage(&storage);
		cvReleaseImage(&frame_bw);
		cvReleaseImage(&frame);
	...
	// timing face landmark
	gettimeofday(&end, NULL); // timing ends for face landmark estimation
    elapsed = (end.tv_sec - begin.tv_sec) + 
              ((end.tv_usec - begin.tv_usec)/1000000.0);
	//cout<<"Landmarks detected in "<<elapsed<<" seconds"<<endl;
	
	gettimeofday(&begin, NULL); // timing function start for writing and rotating the face.
	...
	/******** Code for rotating landmarks after rotating the image *********/
	...
	/***************** Code for rotating landmarks ends here ***************/
	...
	gettimeofday(&end, NULL); // done with image and landmark rotating.
...
void Detector::detectNorm(const Mat& capframe, const float width, const float height, const float patchSize, int maxNumFaces, int& numFaces, Mat* ret, Mat* landmarks_origin, Mat* landmarks, bool mirror, int numLandmarks, bool showLandmark){
	...
	IplImage* frame = &(IplImage) capframe;
	if (frame == NULL)
    {
