...
names1 = dir('./lfpw/testset/*.png');
names2 = dir('./lfpw/testset/*.pts');

gg = 1; % choose image gg to fit
% Modified by Visesh Chari
% input_image = imread(['./lfpw/testset/' names1(gg).name]);
snum = 609 ;
...
input_image = imread( sprintf( ['~/FaceAnimation/JNData/' mapping{snum}]) ) ;
pts = read_shape(['./lfpw/testset/' names2(gg).name], cAAM.num_of_points);
...
% Code by Visesh Chari to load an image and corresponding output by Zhu-Ramanan and transform using
% procrustes to initialize the current_shape variable.
map = [60:-1:52 61:68 16:20 31:-1:27 9 8 7 6 3 2 1 4 5 15 14 13 10 11 12 21 24 25 26 23 22 ...
35 34 33 32 39 40 41 44 46 51 48 50 36 37 38 47 43 45 42 49] ;

imap(map) = 1:68 ;
shp = [(bounding_boxes{snum}.xy(:,1)+bounding_boxes{snum}.xy(:,3))/2 ...
		(bounding_boxes{snum}.xy(:,2)+bounding_boxes{snum}.xy(:,4))/2] ;

[d,z,t] = procrustes( shp(map, :), s0 ) ;
current_shape = t.b*reshape(s0, cAAM.num_of_points, 2)*t.T + t.c ;
...
% current_shape = (1/1.55)*reshape(s0, cAAM.num_of_points, 2) + ...
% 			repmat([size(input_image, 2) size(input_image, 1)]/2, cAAM.num_of_points, 1) ;
...
%% Modified by Visesh Chari on 5th Feb 2015. Commented all lines below since they are only useful for evaluation.
...
% end
% Modified by Visesh Chari 04 Feb 2015, uncomment to get comparison with ground truth.
