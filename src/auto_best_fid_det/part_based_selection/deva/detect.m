function [boxes, resp, components] = detect(input, model, thresh)
    ...
    for rlevel = levels(randperm(length(levels))),
        ...
        for k = 1:numparts,
            ...
            if isempty(resp{level}),
                ...
                resp{level} = merge_resp(our_resp{level}, resp{level});
            end
        ...
        if(max(rscore(:)) > max_score)
            max_score = max(rscore(:));
        end
