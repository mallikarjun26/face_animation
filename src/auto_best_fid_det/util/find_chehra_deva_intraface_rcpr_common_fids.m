function find_chehra_deva_intraface_rcpr_common_fids(path)
    ...
    chehra_deva_intraface_rcpr_common_fids = zeros(size(common_lfpw,1),8);
    ...
    for i=1:size(common_lfpw,1)
        ...
        chehra_deva_intraface_rcpr_common_fids(i,:) = [double(chehra_lfpw{1,1}(ind_c,1)) double(deva_lfpw{1,1}(ind_d,1)) double(intraface_lfpw{1,1}(ind_i,1)) double(rcpr_lfpw{1,1}(ind_r,1)) double(ind_dl) double(ind_dr) double(par_common_lfpw) double(0)];
